...
if page == "Dashboard ภาพรวม":
    ...
    # Mockup Data (ในอนาคตเปลี่ยนเป็น pd.read_csv หรือ Google Sheets API)
...
# ================= LEARNING PAGE =================
    ...
    # เพิ่ม Tab 4 และ 5 ตามที่ขอ
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "1. Heat Balance Analysis", 
        "2. Efficiency (kW/RT)", 
        "3. Sensor Calibration",
        "4. รู้จักตัวแปร CQ1-CQ7",        # เพิ่มใหม่
        "5. ขั้นตอนสำรวจหน้างาน (Audit)"   # เพิ่มใหม่
    ])
    ...
    # --- ส่วนที่เพิ่มใหม่: Tab 4 เรื่อง CQ Parameter ---
    with tab4:
    ...
    # --- ส่วนที่เพิ่มใหม่: Tab 5 เรื่องขั้นตอนสำรวจหน้างาน ---
    with tab5:
...
# ================= QUIZ PAGE =================
    ...
    if "current_quiz" not in st.session_state or st.session_state.quiz_topic != topic:
        ...
        st.session_state.current_quiz = quiz_data[topic] # โหลดคำถาม 10 ข้อของหัวข้อนั้น
        st.session_state.score = 0
        st.session_state.submitted = False

    # สร้าง Form เพื่อไม่ให้หน้าเว็บรีเฟรชตอนกดเลือกช้อยส์
    with st.form("quiz_form"):
        user_answers = {}
        for i, q_item in enumerate(st.session_state.current_quiz):
            st.markdown(f"**ข้อที่ {i+1}: {q_item['q']}**")
            # Radio button สำหรับเลือกคำตอบ
